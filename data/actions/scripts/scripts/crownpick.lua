...
function onUse(cid, item, frompos, item2, topos)
	...
	if grounditem.itemid == 351 and grounditem.actionid == 2123 then
           doTransformItem(grounditem.uid,383)
           doSendMagicEffect(npos,2)
                      totals ={topos = topos}
           addEvent(fechar, tempo, totals)

	elseif grounditem.itemid == 352 and grounditem.actionid == 2123 then
           doTransformItem(grounditem.uid,383)
           doSendMagicEffect(npos,2)
                      totals ={topos = topos}
           addEvent(fechar, tempo, totals)
	
	elseif grounditem.itemid == 353 and grounditem.actionid == 2123 then
           doTransformItem(grounditem.uid,383)
           doSendMagicEffect(npos,2)
                      totals ={topos = topos}
           addEvent(fechar, tempo, totals)
	
	elseif grounditem.itemid == 354 and grounditem.actionid == 2123 then
           doTransformItem(grounditem.uid,383)
           doSendMagicEffect(npos,2)
                      totals ={topos = topos}
           addEvent(fechar, tempo, totals)
	
	elseif grounditem.itemid == 355 and grounditem.actionid == 2123 then
           doTransformItem(grounditem.uid,383)
           doSendMagicEffect(npos,2)
           totals ={topos = topos}
           addEvent(fechar, tempo, totals)
